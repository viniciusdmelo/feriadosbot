...
url_ano_atual = 'https://www.feriados.com.br/feriados-fortaleza-ce.php'
...
feriados_ce_ano_atual = []
for data in feriados_ano_atual:
    feriado = data.text
    feriados_ce_ano_atual.append(feriado)

facultativos_ano_atual = html_feriados_ano_atual.findAll('span', {'class':'style_lista_facultativos'})
facultativos_ce_ano_atual = []
for data in facultativos_ano_atual:
    facultativo = data.text
    facultativos_ce_ano_atual.append(facultativo)

datas_comemorativas_ano_atual = feriados_ce_ano_atual + facultativos_ce_ano_atual
...
descricao_feriado_ce = tabela_final.loc[tabela_final['Data'] == prox_feriado, 'Comemoração'].iloc[0]
prox_feriado_formatado_ce = prox_feriado.strftime('%d/%m/%Y')
