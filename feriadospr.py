...
url_ano_atual = 'https://www.feriados.com.br/feriados-curitiba-pr.php'
...
feriados_pr_ano_atual = []
for data in feriados_ano_atual:
    feriado = data.text
    feriados_pr_ano_atual.append(feriado)

facultativos_ano_atual = html_feriados_ano_atual.findAll('span', {'class':'style_lista_facultativos'})
facultativos_pr_ano_atual = []
for data in facultativos_ano_atual:
    facultativo = data.text
    facultativos_pr_ano_atual.append(facultativo)

datas_comemorativas_ano_atual = feriados_pr_ano_atual + facultativos_pr_ano_atual
...
descricao_feriado_pr = tabela_final.loc[tabela_final['Data'] == prox_feriado, 'Comemoração'].iloc[0]
prox_feriado_formatado_pr = prox_feriado.strftime('%d/%m/%Y')

print(f'O próximo feriado é {descricao_feriado_pr}, em {prox_feriado_formatado_pr}.')
